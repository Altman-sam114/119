...
### Agent C：验收、文档更新与版本提交

Agent C 负责验收 Agent B 的结果，更新核心逻辑文档，并在最终通过后按版本号提交本轮变更。
...
8. 若验收不通过，输出问题清单并明确退回 Agent B 修正，不得提交版本。
9. 若验收通过，执行版本收尾：
   - 确认 `git status --short`，只纳入本轮相关文件。
   - 按版本号提交，例如 `v0.2: 规范 Agent C 版本提交`。
   - 提交说明必须简要概括本版本做了什么，正文可用 2-4 行列出核心工作和验证结果。
   - 提交后输出 commit hash、版本号、工作内容概括、测试命令和结果。
...
- 若核心逻辑、测试规范、入口文档或协作流程变化，必须同步更新相关文档，并由 Agent C 通过后提交对应版本。
...
- 若通过：git commit hash、版本号、提交说明摘要、工作内容概括。
- 建议下一步。
