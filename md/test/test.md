...
```text
RomeLegions-ci-v0.30-main-<short_sha>-run<run_id>-attempt<run_attempt>
```
...
- `BattleView`、`GameViewModel` UI 派生数据、AI 作战计划读板、敌方将领威胁读板、敌情反制建议读板、反制落点/目标地图叠层、反制指令聚焦与执行预览、反制命令链高亮、本方将领协同读板、机动落点读板、战线压力读板、战场焦点读板、地图控制读板、威胁热区叠层、战术命令建议读板、城市经营读板、招募按钮、地图叠层、将领卡、战术姿态按钮或战斗页布局变化。
...
- 渲染前应断言首要反制地图叠层存在，反制路线线段、按位置索引 overlay 和 overlay positions 非空，且包含首要反制建议的推荐落点和威胁目标；失败会抛出 `missingCountermeasureOverlay`。
- 渲染前应断言首要反制指令预览存在，关联首要反制建议，回应单位、推荐姿态、落点、目标、下一步、命令链短标签、姿态 cue、移动 cue、攻击 cue、按钮文案、步骤和无障碍文案可读；调用 `focusCountermeasure(_:)` 后应选中回应军团、记录聚焦反制 ID、聚焦其位置、刷新选中单位姿态预览并显示反制 banner；若反制目标当前可攻击，应能通过 ViewModel 判断攻击按钮为反制目标；失败会抛出 `missingCountermeasureCommandPreview`。
...
- 三尺寸 PNG 和对应 `*-unit.png` 只用于本地目视检查和云端 artifact 复判，确认计划 chip、计划卡/行、敌将 chip、敌将卡/行、反制 chip、反制卡/行、反制指令预览、反制命令链 cue、定位回应按钮、将令 chip、将令卡/行、机动 chip、机动卡/行、战线压力 chip、热区 chip、焦点 chip、军团编制 chip、成长 chip、军议 chip、城市读板、扩建预览、招募按钮、地图热区叠层、机动落点叠层、反制落点/目标叠层、战场焦点卡、热区卡、军团编制卡、军团成长决策卡、军团成长优先级行、战术建议卡、将领读板、战功、姿态预览、敌军路线、本方建议路线、反制引导线、目标叠层、反制攻击按钮和命令入口没有明显裁切、重叠或遮挡；PNG 不提交版本库。
...
- `Tools/RenderBattlePreview/main.swift` 渲染前还会断言 `primaryCountermeasureMapOverlay`、`countermeasureRouteSegments`、`countermeasureOverlaysByPosition` 和 `countermeasureOverlayPositions` 可用，且包含首要反制建议的推荐落点和威胁目标；断言失败会抛出 `missingCountermeasureOverlay`。
- `Tools/RenderBattlePreview/main.swift` 渲染前还会断言 `primaryCountermeasureCommandPreview` 可用，且文案、步骤、按钮、命令链短标签、姿态 cue、移动 cue、攻击 cue 和聚焦行为可读；调用 `focusCountermeasure(_:)` 后应选中回应军团、记录聚焦反制 ID、更新焦点位置、暴露推荐姿态预览并显示反制 banner；若反制目标当前可攻击，应能标记当前攻击按钮对应目标；断言失败会抛出 `missingCountermeasureCommandPreview`。
