...
- v0.52 起地图主导壳层像素检查必须覆盖三尺寸单位/城市场景的中央地图、右侧五段边缘工具和底部命令坞；任一关键区域缺失应抛出 `missingMapDominantBattleShell`。命令坞只采样真实底部区域，单位场景必须出现攻击/技能操作色，城市场景必须出现扩建/招募操作色，且两者像素签名必须不同；空坞或沿用旧侧栏布局不得通过。
...
- `Tools/RenderBattlePreview/main.swift` 还会对每张图检查地图主导壳层：顶部以下中央地图必须有足够地形色彩，右侧五个工具按钮所在纵向分段必须逐段可见；失败会抛出 `missingMapDominantBattleShell`。底部命令坞只采 `NSBitmapImageRep` 的真实底部区域，单位场景要求红色攻击与青色技能像素，城市场景要求金橙色扩建/招募像素，二者还必须通过 `missingDistinctCommandDockRender` 差异门禁。
