...
```text
RomeLegions-ci-v0.52-main-<short_sha>-run<run_id>-attempt<run_attempt>
```
...
- v0.52 起地图主导壳层像素检查必须覆盖三尺寸单位/城市场景的中央地图、按顶部锚点逐一定位的五个 44×44 右侧工具按钮和底部命令坞；`NSBitmapImageRep.colorAt` 按顶部原点读取，工具轨直接使用顶部坐标，命令坞采样区换算到图像底部。任一关键区域缺失应抛出 `missingMapDominantBattleShell`。单位预览在完成全部只读语义断言后加入专用相邻敌军，确保命令坞必须出现攻击/技能操作色；城市场景必须出现扩建/招募操作色，且两者像素签名必须不同。空坞、缺少窄屏休整/跳过、沿用旧侧栏布局或仅凭比例猜测工具轨位置均不得通过。
...
- `Tools/RenderBattlePreview/main.swift` 还会对每张图检查地图主导壳层：顶部以下中央地图必须有足够地形色彩，右侧五个工具按钮按 `NSBitmapImageRep` 顶部原点与薄顶栏后的真实 44×44 布局逐个采样并输出像素诊断；失败会抛出 `missingMapDominantBattleShell`。底部命令坞换算到同一顶部原点后的真实图像底部区域；单位场景在渲染前加入不参与前序语义断言的相邻攻击夹具，要求红色攻击与青色技能像素，城市场景要求金橙色扩建/招募像素，二者还必须通过 `missingDistinctCommandDockRender` 差异门禁。
