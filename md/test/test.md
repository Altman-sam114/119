...
- v0.52 起地图主导壳层像素检查必须覆盖三尺寸单位/城市场景的中央地图、右侧边缘工具和底部命令坞；任一关键区域缺失应抛出 `missingMapDominantBattleShell`。原紧凑命令区和城市读板检查改为采样底部命令坞，阈值不得低于 v0.51 基线。
...
- `Tools/RenderBattlePreview/main.swift` 还会对每张图检查地图主导壳层：顶部以下中央地图必须有地形色彩，右侧边缘工具必须可见，底部军团/城市命令坞必须有足够可见内容；失败会抛出 `missingMapDominantBattleShell`。由于 `NSBitmapImageRep` 坐标方向可能与 SwiftUI 逻辑坐标相反，底部命令坞采用上下对称候选区域采样，但亮像素阈值不下调。
