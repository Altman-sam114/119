...
4. `BattleView` 读取 `GameViewModel` 的派生数据：当前回合、资源、选中单位、选中城市、城市经营与招募读板、可移动格、攻击目标、战斗预览、将领技能预览、选中单位指挥简报、军团编制摘要、军团成长决策摘要、军团成长优先级摘要、本方将领协同摘要、机动落点摘要、战术命令建议、战术姿态预览、战功状态、任务目标、战役状态、敌军意图摘要、AI 作战计划摘要、敌方将领威胁摘要、敌情反制建议摘要、反制指令预览与命令链高亮、敌军意图路线/目标地图叠层、机动落点地图叠层、反制落点/目标地图叠层、主动地图叠层图例、战线压力摘要、战场焦点摘要、地图控制摘要、威胁热区摘要和战局态势。
...
- `GameViewModel.countermeasureCommandPreviews`、`primaryCountermeasureCommandPreview`、`selectedCountermeasureCommandPreview` 和 `focusedCountermeasureID` 继续把反制建议转成只读指令预览，说明推荐姿态、落点是否可达、目标是否可直接攻击、命令链短标签和阻塞原因；`focusCountermeasure(_:)` 只改变 ViewModel 选择态、位置、聚焦 ID 和 banner，使现有可达格、攻击目标和姿态预览自然刷新，不移动单位、不攻击、不切换姿态，也不改变 `GameState`。
- `BattleView` 在 `TacticalOrderControlView` 中用 `selectedCountermeasureCommandPreview` 标记推荐姿态按钮，在完整/紧凑 `ActionsPanelView` 中标记当前可攻击的反制目标按钮；这些高亮只解释现有命令入口，按钮 action 和 disabled 规则仍走原 `GameViewModel` 命令方法。
...
- `CountermeasureCommandPreview`：`GameViewModel` 的反制指令 UI 派生数据，供敌情反制卡、战局反制行和选中回应军团后的军令面板展示推荐姿态、落点、目标、下一步、阻塞原因、命令链短标签、姿态按钮 cue 和攻击按钮 cue。
...
- 敌情反制建议读板、指令预览和命令链高亮不得改变 `GameState`、AI 评分、技能释放、攻击预览、移动、姿态或真实 AI 决策；反制建议只能链接现有敌方威胁和本方战术/机动/将令报告，聚焦方法只改变 ViewModel 选择态，按钮高亮只解释既有命令入口，不自动执行。
...
- 预览渲染：`Tools/RenderBattlePreview/main.swift` 生成战斗页 PNG，并在渲染前断言敌军意图六边形邻接路径、目标格、预计伤害、主动地图叠层图例、AI 作战计划摘要、敌方将领威胁摘要、敌情反制建议摘要、反制地图叠层、反制指令预览、命令链高亮 cue 与聚焦、战线压力摘要、战场焦点摘要、地图控制摘要、威胁热区摘要、军团编制摘要、军团成长决策摘要、军团成长优先级摘要、本方将领协同摘要、机动落点摘要/地图 overlay、战术建议摘要/路径/目标、选中单位将领详情、被动贡献、战功摘要、战术姿态预览和城市经营/招募读板存在；每个尺寸会输出城市场景图和同尺寸 `*-unit.png` 单位场景图。
