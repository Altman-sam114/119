...
4. `BattleView` 读取 `GameViewModel` 的派生数据：当前回合、资源、选中单位、选中城市、可移动格、攻击目标、战斗预览、将领技能预览、选中单位指挥简报、战术姿态预览、战功状态、任务目标、战役状态、敌军意图摘要、敌军意图路线/目标地图叠层、战局态势。
...
- `GameViewModel.enemyIntentSummaries` 把 `AIIntent`、来源单位、目标单位和目标城市转成 UI 文案；`enemyIntentMapOverlays` 再派生起点、目的地、目标格、影响文案和 `origin -> destination -> target` 可视线段。
- `BattleView` 只消费这些派生集合，在地图上显示敌军意图路线、目的地叠层和目标格叠层；这些线段只是意图可视化，不是新的寻路或 AI 规划。
...
- 敌军意图地图叠层复用 `AIIntent.destination`、`targetUnitID`、`targetCityID` 和 `projectedDamage`，由 `GameViewModel` 派生，不会重新评分、重新选择目标或改变真实 AI 行为。
...
- 预览渲染：`Tools/RenderBattlePreview/main.swift` 生成战斗页 PNG，并在渲染前断言敌军意图路线、目标格、预计伤害、选中单位将领详情、被动贡献、战功摘要和战术姿态预览存在。
...
- 敌军意图地图叠层只能展示 `AIIntent` 既有字段派生出的起点、目的地和目标，不得反向改变 AI 决策或核心状态。
