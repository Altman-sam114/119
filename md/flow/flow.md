...
- `GameViewModel.primaryBattleObjectiveChainSummary` 只读组合首要战场焦点、当前将令协同、首要机动落点和选中单位军议，输出“1 焦点、2 将令、3 机动、4 军议”的战场目标线、紧凑文案、优先级文案和无障碍说明；`primaryBattleObjectiveMapOverlay`、`battleObjectiveRouteSegments`、`battleObjectiveOverlaysByPosition` 和 `battleObjectiveOverlayPositions` 再把同一目标线派生为地图阶段标记和连线；它们不调用 `GameState` 写命令，也不改变焦点、将令、机动或军议的评分排序。
- `BattleView` 在战场面板展示战场目标链路，并在地图上显示“1 焦点、2 将令、3 机动、4 军议”阶段标记和金色目标线；焦点、将令、机动和军议卡片展示各自链路 cue；这些 cue 和叠层只解释当前目标线，不自动移动、攻击、发动技能或切换姿态。
...
- `BattleObjectiveChainSummary` / `BattleObjectiveMapOverlay`：`GameViewModel` 的战场目标链路 UI 派生数据，组合焦点、将令、机动和军议摘要，供战场面板、相关卡片、地图阶段标记和目标线连线展示只读 cue。
...
- 战场目标链路和目标线地图叠层不得改变 `GameState`、焦点评分、将令评分、机动评分、战术建议、移动、攻击、技能、姿态或 AI 决策；它只能组合已有 ViewModel 只读摘要并展示阶段位置。
- 战斗预览和实际战斗必须共享规则来源。
...
- 预览渲染：`Tools/RenderBattlePreview/main.swift` 生成战斗页 PNG，并在渲染前断言敌军意图六边形邻接路径、目标格、预计伤害、主动地图叠层图例、AI 作战计划摘要、敌方将领威胁摘要、敌情反制建议摘要、反制地图叠层、反制指令预览、命令链高亮 cue、焦点链路与聚焦、战线压力摘要、战场焦点摘要、战场目标链路、战场目标线地图叠层、地图控制摘要、威胁热区摘要、军团编制摘要、军团成长决策摘要、军团成长优先级摘要、本方将领协同摘要、机动落点摘要/地图 overlay、战术建议摘要/路径/目标、选中单位将领详情、被动贡献、战功摘要、战术姿态预览和城市经营/招募读板存在；每个尺寸会输出城市场景图和同尺寸 `*-unit.png` 单位场景图。
