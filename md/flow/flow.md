...
- `BattleView` 在战场面板展示战场目标链路，并在地图上显示“1 焦点、2 将令、3 机动、4 军议”阶段标记和金色目标线；目标线卡片可定位各阶段并展示阶段命令预览，完整/紧凑军令面板会展示选中罗马单位关联的目标线阶段命令预览；地图徽标、阶段定位按钮、推荐姿态按钮、当前可攻击目标和将领技能入口会读取同一 active/selected 阶段 cue 做联动高亮；反制 cue 与反制按钮高亮仍优先于目标线 cue；这些 cue、叠层、定位和预览只解释当前目标线，不自动移动、攻击、发动技能或切换姿态。
...
- 战场目标链路、目标线地图叠层、阶段聚焦、阶段命令预览和联动高亮不得改变 `GameState`、焦点评分、将令评分、机动评分、战术建议、移动、攻击、技能、姿态或 AI 决策；它只能组合已有 ViewModel 只读摘要、展示阶段位置、调整 UI 聚焦态并提示既有命令入口。
- 战斗预览和实际战斗必须共享规则来源。
...
- 预览渲染：`Tools/RenderBattlePreview/main.swift` 生成战斗页 PNG，并在渲染前断言敌军意图六边形邻接路径、目标格、预计伤害、主动地图叠层图例、AI 作战计划摘要、敌方将领威胁摘要、敌情反制建议摘要、反制地图叠层、反制指令预览、命令链高亮 cue、焦点链路与聚焦、战线压力摘要、战场焦点摘要、战场目标链路、战场目标线地图叠层、目标线阶段聚焦、阶段命令预览、阶段联动高亮 cue、地图控制摘要、威胁热区摘要、军团编制摘要、军团成长决策摘要、军团成长优先级摘要、本方将领协同摘要、机动落点摘要/地图 overlay、战术建议摘要/路径/目标、选中单位将领详情、被动贡献、战功摘要、战术姿态预览和城市经营/招募读板存在；每个尺寸会输出城市场景图和同尺寸 `*-unit.png` 单位场景图。
