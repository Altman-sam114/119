...
4. `BattleView` 读取 `GameViewModel` 的派生数据：当前回合、资源、选中单位、选中城市、可移动格、攻击目标、战斗预览、将领技能预览、任务目标、战役状态、敌军意图、战局态势。
...
- `generalSkillPreview(unitID:)` 只读计算技能范围、受影响友军/敌城、预计恢复量、预计城防削弱和不可用原因，不改变原始 `GameState`。
- `useGeneralSkill(unitID:)` 处理鹰旗鼓舞、攻城布阵、战地补给、盾墙号令等技能，并复用技能预览的目标筛选逻辑后消耗行动。
- `GameViewModel.selectedGeneralSkillPreview` 将核心预览转为地图范围、目标集合和按钮摘要；SwiftUI 只展示范围叠层、目标叠层、将领卡和命令 detail，不复制技能规则。
...
- `aiIntents(for:limit:)` 只预测敌军倾向，不改变状态；攻击类意图的预计伤害来自规划态战斗预览。
- AI 主动技能判断和 `.useSkill` 意图复用将领技能预览；攻城技能填入目标城市，治疗类技能填入主要受益友军。
...
- `GeneralSkillPreview`：将领主动技能只读预览，包含范围、目标、预计恢复、预计削城防、可执行状态和 UI 摘要。
