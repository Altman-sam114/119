...
- `CoastlineBuilder` 只读遍历 `state.tiles`，对每个水格找出非水相邻格产出 `CoastlineSegment`，`CoastlineLayerView` 在地貌层与路线层之间用 Canvas 沿两格中点绘制沙色岸线与浅色浪缘，随镜头统一变换；`HexTileView` 默认描边减淡并按坐标做确定性明度微调，`ThreatHeatTileOverlay` 只在热区中心格显示等级图标，`MapControlTileOverlay` 使用低对比细实线。这些均为纯展示派生，不写入 `GameState`、报告、命令或存档。
